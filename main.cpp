...
Hasher<int> *INT_HASHER = [](const int &key) { return key; };

Hasher<string> *STRING_HASHER = [](const string &key) {
    int hash = 0;
...
int main()
{
    ...
    HashSet<int> intHashSet(INT_HASHER);
    testSet(intHashSet, 1, 5);

    test::section("HashSet<string>");
    HashSet<string> stringHashSet(STRING_HASHER);
    ...
    HashMap<int, int> intHashMap(INT_HASHER);
    testMap(intHashMap, 1, 2, 3, 4);

    test::section("HashMap<string, float>");
    HashMap<string, float> strFloatHashMap(STRING_HASHER);
